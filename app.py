...
def load_roster_state(filename: str = "default_roster.json"):
    """Load saved roster state from disk. Supports both simple dict and raw VFL API schemas. Returns (player_names, igl_name, player_roles_map)."""
    os.makedirs(ROSTER_STATES_DIR, exist_ok=True)
                ...
                if raw_p_list and isinstance(raw_p_list[0], str):
                    ...
                    for item in raw_p_list:
                        ...
                        if p_name:
                            ...
                            role_num = item.get("eventPlayer", {}).get("playerRole") if "eventPlayer" in item else item.get("playerRole")
                            role_str = VFL_ROLE_ENUM_MAP.get(role_num, "Flex") if role_num is not None else item.get("role", "Flex")
                            if role_str == "Flex":
                                role_str = "Wildcard"
                            is_wc = bool(item.get("isWildcard") or item.get("is_wildcard"))
                            
                            roles_map[p_name] = {
                                "role": role_str,
                                "is_wildcard": is_wc
                            }
                    return extracted_names, extracted_igl or state.get("igl", None), roles_map
        except Exception:
            pass
    return [], None, {}


def fetch_vfl_roster_by_user_id(user_id: str | int, event_id: int = 10, gameweek: int = 0) -> tuple[list[str], str | None, dict | None]:
    """
    Fetches user fantasy roster from VFL API endpoint:
    https://api.valorantfantasyleague.net/api/fantasyteam/team?userId={user_id}&eventId={event_id}&gameweek={gameweek}
...
def fetch_vfl_rosters_all_weeks(user_id: str | int, event_id: int = 10) -> dict[int, dict]:
    """
    Scans VFL API endpoints starting at gameweek=1 and incrementing until HTTP 404 is encountered.
    Saves each week's roster JSON to disk with filename format: {user_id}_week_{week_no}_roster.json.
    Returns a dict mapping gameweek_num -> raw_json_payload.
    """
    import urllib.request
    import urllib.error
    ...
    for gw in range(1, 20):
        url = f"https://api.valorantfantasyleague.net/api/fantasyteam/team?userId={user_id_clean}&eventId={event_id}&gameweek={gw}"
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"})
        try:
            with urllib.request.urlopen(req, timeout=10) as response:
                if response.status == 200:
                    ...
                    if players:
                        weeks_found[gw] = data
                        out_fname = f"{user_id_clean}_week_{gw}_roster.json"
                        out_path = os.path.join(ROSTER_STATES_DIR, out_fname)
                        with open(out_path, "w", encoding="utf-8") as f_out:
                            json.dump(data, f_out, indent=2)
                        
                        # Save legacy alias file as well
                        legacy_fname = f"user_{user_id_clean}_roster.json"
                        legacy_path = os.path.join(ROSTER_STATES_DIR, legacy_fname)
                        with open(legacy_path, "w", encoding="utf-8") as f_leg:
                            json.dump(data, f_leg, indent=2)
                    else:
                        break
                else:
                    break
            ...
            if e.code == 404:
                logger.info(f"VFL scan for User {user_id_clean} reached 404 at Gameweek {gw}. Scan complete.")
                break
            else:
                logger.warning(f"VFL scan for User {user_id_clean} Gameweek {gw} failed with code {e.code}")
                break
        except Exception as ex:
            logger.warning(f"VFL scan for User {user_id_clean} Gameweek {gw} error: {ex}")
            break
...
for f in files:
    try:
        ...
        if not match_id or not team_a or not team_b:
            continue
        event = norm.get("event") or "Unknown Event"
        date_str = norm.get("date") or "Unknown Date"
...
_saved_players, _saved_igl, _ = load_roster_state()
...
def execute_gpp_optimization_pipeline(opt_ruleset_val, opt_depth_val, opt_salary_cap_val, player_global_stats_val, vfl_scraper_obj):
    with st.spinner("Initializing GPP Optimization Pipeline..."):
        try:
            from pathlib import Path
            root_dir = Path(__file__).resolve().parent
            ...
            is_intl_rules = "International" in opt_ruleset_val
            ...
            gpp_matchup_pairs = []
            try:
                gpp_sched_res = vfl_scraper_obj.get_schedule(gameweek=int(gpp_gw), event_id=int(gpp_ev))
            ...
            df_meta_slate, df_fused_slate = prepare_player_slate(
                num_iterations=opt_depth_val, 
                allowed_teams=active_pool,
                matchup_pairs=gpp_matchup_pairs,
                h2h_stats=h2h_db,
                player_global_stats=player_global_stats_val
            )
            opt_solution = solve_vfl_knapsack(
                df_meta_slate, 
                salary_cap=opt_salary_cap_val,
...
if btn_generate_lineup:
    execute_gpp_optimization_pipeline(opt_ruleset, opt_depth, opt_salary_cap, player_global_stats, vfl_scraper_inst)
...
with tab_match:
    ...
    selected_match = matches_lookup.get(selected_match_id) if selected_match_id else None
    if not selected_match and match_options:
        selected_match = matches_lookup.get(match_options[0][0])
        
    if selected_match:
        ma_team_a = selected_match["team_a"]
        ma_team_b = selected_match["team_b"]
        norm_match = selected_match.get("norm", {})
    else:
        ma_team_a, ma_team_b = "Fnatic", "Paper Rex"
        norm_match = {}
    ...
    actual_maps_data = norm_match.get("maps", [])
    maps_score = norm_match.get("maps_score", {})
    actual_team_a_score = maps_score.get("team_a", norm_match.get("score_a", "?"))
    actual_team_b_score = maps_score.get("team_b", norm_match.get("score_b", "?"))
    actual_winner_idx = None
    actual_winner_name = norm_match.get("winner") or "N/A"
...
with tab_optimizer:
    ...
    with c_hdr2:
        btn_tab_inline_gen = st.button("⚡ Run Optimizer", type="primary", key="btn_tab_inline_gen", width="stretch")

    if btn_tab_inline_gen:
        execute_gpp_optimization_pipeline(opt_ruleset, opt_depth, opt_salary_cap, player_global_stats, vfl_scraper_inst)
    ...
    if "gpp_solution" in st.session_state and st.session_state.get("gpp_generated", False):
        ...
        opt_lineup = sol.get("selected_players") or sol.get("lineup") or []
        st.markdown(f"#### 👥 Optimal {len(opt_lineup)}-Man Tournament Lineup")
        
        # Use dynamic rows of 3 columns to support different lineup sizes
        cols_per_row = 3
        cols = None
        for idx, p in enumerate(opt_lineup):
        ...
        st.info("💡 Click **'⚡ Run Optimizer'** above or in the sidebar Command Center to compute the GPP optimal roster!")
...
with tab_transfers:
    ...
    saved_names, saved_igl, saved_roles_map = load_roster_state(active_roster_file)
    ...
    if "roster_ms_initialized" not in st.session_state or st.session_state.get("force_roster_reload", False):
        ...
        # 1. Place explicitly flagged Wildcards first if from VFL JSON payload
        for name in valid_saved_names:
            r_info = saved_roles_map.get(name, {})
            if r_info.get("is_wildcard"):
                if len(saved_by_role["Wildcard"]) < wildcard_max:
                    saved_by_role["Wildcard"].append(name)
                    used_names.add(name)

        # 2. Place core role players based on extracted role (or slate_lookup fallback)
        for role in ["Duelist", "Initiator", "Controller", "Sentinel"]:
            for name in valid_saved_names:
                if name not in used_names:
                    r_info = saved_roles_map.get(name, {})
                    p_role = r_info.get("role") or slate_lookup.get(name, {}).get("role")
                    if p_role == role and len(saved_by_role[role]) < core_max:
                        saved_by_role[role].append(name)
                        used_names.add(name)

        # 3. Fill remaining slots into core or Wildcard
        for name in valid_saved_names:
            if name not in used_names:
                r_info = saved_roles_map.get(name, {})
                p_role = r_info.get("role") or slate_lookup.get(name, {}).get("role")
                if p_role in saved_by_role and len(saved_by_role[p_role]) < core_max:
                    saved_by_role[p_role].append(name)
                    used_names.add(name)
                elif len(saved_by_role["Wildcard"]) < wildcard_max:
                    saved_by_role["Wildcard"].append(name)
                    used_names.add(name)
    ...
    c_hdr_ta1, c_hdr_ta2 = st.columns([3, 1])
    with c_hdr_ta1:
        st.markdown(f"##### 📋 Select Active Roster Categories ({req_size} Players Total)")
    with c_hdr_ta2:
        if st.button("⚡ Auto-Fill Saved Roster", key="btn_auto_fill_saved_roster", width="stretch", help="Populate category dropdowns from active saved roster state"):
    ...
    st.markdown("##### 🌐 Fetch Roster via VFL User ID (All Gameweeks)")
    st.caption(r"Fetch your fantasy team directly from official VFL API (`https://api.valorantfantasyleague.net`). Scans gameweeks automatically until 404.")
    ...
    with c_vfl2:
        ...
        st.write("")
        if st.button("🌐 Fetch VFL Rosters (All Weeks)", key="btn_fetch_vfl_user_id", width="stretch"):
            uid_clean = vfl_uid_val.strip()
            if uid_clean:
                with st.spinner(f"Scanning VFL gameweeks for User ID {uid_clean}..."):
                    try:
                        w_found = fetch_vfl_rosters_all_weeks(uid_clean, event_id=10)
                        if w_found:
                            max_gw = max(w_found.keys())
                            latest_fname = f"{uid_clean}_week_{max_gw}_roster.json"
                            f_names, f_igl, _ = load_roster_state(latest_fname)
                            st.session_state["saved_roster_names"] = f_names
                            st.session_state["saved_igl_name"] = f_igl
                            st.session_state["active_roster_file"] = latest_fname
                            st.session_state["prev_selected_roster_file"] = latest_fname
                            st.session_state["force_roster_reload"] = True
                            st.toast(f"Fetched {len(w_found)} weekly rosters for User ID {uid_clean}! Active roster set to Week {max_gw}.", icon="🌐")
                            st.rerun()
                        else:
                            st.warning(f"No valid gameweek rosters found for VFL User ID {uid_clean}.")
                    except Exception as ex:
                        st.error(f"Failed to fetch VFL rosters for User ID {uid_clean}: {ex}")
            else:
                st.warning("Please enter a valid VFL User ID.")

    # Gameweek Roster Selector Buttons for active User ID
    uid_current = vfl_uid_val.strip()
    if uid_current:
        import re
        saved_files_all = list_saved_roster_files()
        gw_files = []
        pattern = re.compile(rf"^{re.escape(uid_current)}_week_(\d+)_roster\.json$")
        for fname in saved_files_all:
        ...
        if gw_files:
            st.markdown(f"###### 📅 Select Active Gameweek Roster for Optimization (User {uid_current}):")
            col_wks = st.columns(len(gw_files))
            for idx, (gw_num, fname) in enumerate(gw_files):
                is_active = (fname == active_roster_file)
                btn_type = "primary" if is_active else "secondary"
                btn_label = f"Week {gw_num}{' (Active)' if is_active else ''}"
                with col_wks[idx]:
                    if st.button(btn_label, key=f"btn_gw_sel_{uid_current}_{gw_num}", type=btn_type, width="stretch"):
                        f_names, f_igl, _ = load_roster_state(fname)
                        st.session_state["saved_roster_names"] = f_names
                        st.session_state["saved_igl_name"] = f_igl
                        st.session_state["active_roster_file"] = fname
                        st.session_state["prev_selected_roster_file"] = fname
                        st.session_state["force_roster_reload"] = True
                        st.toast(f"Set Gameweek {gw_num} roster ('{fname}') as active roster for optimization!", icon="⚡")
                        st.rerun()
    ...
    if selected_file_to_load != "➕ Create New JSON File..." and selected_file_to_load != prev_file:
        ...
        loaded_players, loaded_igl, _ = load_roster_state(selected_file_to_load)
        if loaded_players:
    ...
    with c_state2:
        ...
        with btn_col_b:
            if st.button("📂 Load State", key="btn_load_roster_state", width="stretch"):
                loaded_players, loaded_igl, _ = load_roster_state(target_filename)
                if loaded_players:
                    st.session_state["saved_roster_names"] = loaded_players
                    st.session_state["saved_igl_name"] = loaded_igl
                    st.session_state["active_roster_file"] = target_filename
    ...
    if btn_calc_trades:
        if num_selected != req_size:
            ...
            with st.spinner("Calculating optimal transfers..."):
                ...
                filtered_slate = [p for p in slate_data if p.get("team") in active_team_pool or p.get("name") in selected_roster]
                
                if "gpp_meta_df" not in st.session_state:
                    from knapsack_solver import prepare_player_slate
                    df_meta_slate, _ = prepare_player_slate(
                        allowed_teams=active_team_pool,
                ...
                for p in filtered_slate:
                    ...
                    p_rows = df_m.iloc[0:0]
                    if "player_name" in df_m.columns:
                        p_rows = df_m[df_m["player_name"] == p_name]
                    elif "name" in df_m.columns:
                        p_rows = df_m[df_m["name"] == p_name]
                    elif "Player" in df_m.columns:
                        p_rows = df_m[df_m["Player"] == p_name]
                    elif "player_id" in df_m.columns and p.get("player_id"):
                        p_rows = df_m[df_m["player_id"] == p.get("player_id")]
                        
                    if not p_rows.empty and "EV" in p_rows.columns:
                        ev_val = float(p_rows.iloc[0]["EV"])
                    elif "ppg" in p and p.get("ppg") is not None:
                        ev_val = float(p["ppg"])
                    elif "computed_ppg" in p and p.get("computed_ppg") is not None:
                        ev_val = float(p["computed_ppg"])
                    elif "avg_points" in p and p.get("avg_points") is not None:
                        ev_val = float(p["avg_points"])
                    else:
                        ev_val = 10.0
                        
                    if p_team in active_team_pool:
                        p["EV"] = ev_val
                        p["computed_ppg"] = ev_val
                    else:
                        p["EV"] = 0.0
                        p["computed_ppg"] = 0.0
    ...
    if vfl_players_data:
        ...
        if not vfl_df.empty:
            ...
            if 'team' not in vfl_df.columns and 'vlr_team_id' in vfl_df.columns:
                vfl_df['team'] = vfl_df['vlr_team_id'].map(id_to_team_name).fillna(vfl_df['vlr_team_id']).astype(str)
            elif 'team' in vfl_df.columns:
                vfl_df['team'] = vfl_df['team'].map(id_to_team_name).fillna(vfl_df['team']).astype(str)
        ...
        with filter_col1:
            team_options = sorted([str(x) for x in vfl_df['team'].dropna().unique()])
            team_filter = st.multiselect("Filter by Team", team_options, key="vfl_team_filter")
        with filter_col2:
            role_options = sorted([str(x) for x in vfl_df['role'].dropna().unique()])
            role_filter = st.multiselect("Filter by Role", role_options, key="vfl_role_filter")

        filtered_df = vfl_df.copy()
        if team_filter:
            filtered_df = filtered_df[filtered_df['team'].astype(str).isin(team_filter)]
        if role_filter:
            filtered_df = filtered_df[filtered_df['role'].astype(str).isin(role_filter)]
        ...
        # Ensure object columns in display_vfl_df are converted to string to prevent ArrowTypeError
        for col in display_vfl_df.columns:
            if display_vfl_df[col].dtype == 'object':
                display_vfl_df[col] = display_vfl_df[col].astype(str)
