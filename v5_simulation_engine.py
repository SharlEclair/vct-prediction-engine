...
def _load_ml_model_pkg(model_name: str = "map_winner_v1"):
    for dir_path in ["models/production", "models/checkpoints"]:
        p = os.path.join(dir_path, f"{model_name}.pkl")
        if os.path.exists(p):
            try:
...
class MapVetoBandit:
            ...
            try:
                ...
                for map_data in norm["maps"]:
                    ...
                    if t1_score > t2_score:
                        ...
                        self.window_plays[window_id][m_name]["wins_a"] += 1
    ...
    def predict_map_win_rate_dr(self, team: str, opponent: str, map_name: str,
        ...
        # Phase 6 ML integration: blend with trained map_winner_v1 ML model if available
        ml_pkg = _load_ml_model_pkg("map_winner_v1")
        if ml_pkg is not None:
            try:
                model = ml_pkg["model"]
                feature_cols = ml_pkg.get("metadata", {}).get("feature_cols", [])
                if feature_cols:
                    # Construct feature vector delta
                    t1_wr = wins / max(1, plays) if plays > 0 else 0.5
                    t2_plays = self.team_plays.get(opponent, {}).get(map_name, 0)
                    t2_wins = self.team_wins.get(opponent, {}).get(map_name, 0)
                    t2_wr = t2_wins / max(1, t2_plays) if t2_plays > 0 else 0.5
                    
                    diff_wr = t1_wr - t2_wr
                    # Build sample input matching feature_cols order
                    sample_dict = {
                        "diff_win_rate": diff_wr,
                        "diff_attack_win_rate": diff_wr * 0.5,
                        "diff_defense_win_rate": diff_wr * 0.5,
                        "avg_map_rounds": 21.5
                    }
                    feat_vec = np.array([[sample_dict.get(c, 0.0) for c in feature_cols]])
                    p_ml = float(model.predict_proba(feat_vec)[0, 1])
                    val = float(0.5 * val + 0.5 * p_ml)
            except Exception:
                pass
...
class VCTv5SimulationEngine:
    ...
    def simulate_match(self, team_a: str, team_b: str, series_type: str = "Bo3", target_patch: str = "9.02", num_iterations: int = 10000, override_maps: list[str] = None, target_date: datetime | None = None, ub_advantage: bool | str = False, veto_priority: str = "team_a") -> dict:
        ...
        roster_a = list(get_simulation_roster(team_a, self.raw_dir))[:5]
        roster_b = list(get_simulation_roster(team_b, self.raw_dir))[:5]
        
        if len(roster_a) < 5 or len(roster_b) < 5:
            logger.warning("Fewer than 5 roster players identified. Falling back to default baseline.")
            default_a = ["something", "aspas", "zekken", "wo0t", "Derke"]
            default_b = ["Leo", "trent", "chronicle", "Sacy", "Boaster"]
            roster_a = (roster_a + default_a)[:5]
            roster_b = (roster_b + default_b)[:5]
...
def get_simulation_historical_stats(raw_dir: str):
    ...
    for m in matches:
        for map_data in m.get("maps", []):
            ...
            players_raw = map_data.get('players', [])
            if isinstance(players_raw, dict):
                p_list = []
                for team_key in ['team1', 'team2']:
                    p_list.extend(players_raw.get(team_key, []))
            ...
            for p in p_list:
                if isinstance(p, str):
                    ...
                    p_name = p.get('name') or p.get('player_name') or p.get('id')
                    agent = p.get('agent', '')
...
def get_simulation_roster(team_name: str, raw_dir: str) -> tuple[str, ...]:
        ...
        try:
            with open(f, "r", encoding="utf-8") as file:
                ...
                if is_ta_match or is_tb_match:
                    ts = norm.get("timestamp") or parse_simulation_match_date(norm.get("date", ""))
                    matches_with_team.append((ts, norm, ta, tb))
        except Exception:
            pass
    ...
    for map_data in latest_seg.get('maps', []):
        players_raw = map_data.get('players', [])
        if isinstance(players_raw, dict):
            p_list = players_raw.get(team_key, [])
        elif isinstance(players_raw, list):
            p_list = players_raw
        else:
            p_list = []
        ...
        for p in p_list:
            ...
            if p_name and "inactive" not in p_name.lower():
                roster.add(p_name)
