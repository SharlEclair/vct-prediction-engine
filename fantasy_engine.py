...
from utils.team_registry import resolve_team_id
...
def parse_match_date(date_str: str) -> datetime:
    ...
    try:
        dt_raw = datetime.strptime(f"{month} {day} {year} {time_str} {ampm}", "%B %d %Y %I:%M %p")
        return dt_raw
    except Exception:
        return datetime(2026, 6, 22)
...
def match_team(token_team: str, team_a: str, team_b: str) -> int:
    """Helper to match player team token to team_a or team_b (returns 1 for team_a, -1 for team_b, 0 for unknown)."""
    if not token_team or not team_a or not team_b:
        return 1
    t_clean = str(token_team).lower().strip()
    ta_clean = str(team_a).lower().strip()
    tb_clean = str(team_b).lower().strip()
    if t_clean in ta_clean or ta_clean in t_clean:
        return 1
    elif t_clean in tb_clean or tb_clean in t_clean:
        return -1
    return 1
    ...
    for f in files:
        try:
            ...
            if datetime(2026, 6, 20) <= ts <= datetime(2026, 6, 30):
                t1_id = resolve_team_id(norm["teams"]["team1"])
                t2_id = resolve_team_id(norm["teams"]["team2"])
                if t1_id and t2_id:
    ...
    for f in files:
        try:
            with open(f, "r", encoding="utf-8") as file:
                content = json.load(file)
            norm = normalize_match(content)
            t1_name = norm["teams"]["team1"]
            t2_name = norm["teams"]["team2"]
            t1_id = resolve_team_id(t1_name) or hash(t1_name) % 10000
            t2_id = resolve_team_id(t2_name) or hash(t2_name) % 10000
...
class VCTFantasyEngine:
    ...
    def __init__(self):
        ...
        try:
            player_stats_path = os.path.join(PROCESSED_DIR, "player_stats.json")
            ...
            if os.path.exists(player_stats_path):
                with open(player_stats_path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    for ps in data["data"]["segments"]:
                        p_name = ps["player"]
                        kpr = float(ps.get("kills_per_round", 0.72))
                        rating = float(ps.get("rating", 1.05))
                        self.baseline_lookup[p_name] = {"kpr": kpr, "rating": rating}
        except Exception:
            pass
    ...
    def score_match_json(self, match_filepath: str) -> list[dict]:
        ...
        norm = normalize_match(data)
        team_a_name = norm.get("team_a", "")
        team_b_name = norm.get("team_b", "")
        ...
        score_a, score_b = 0, 0
        for m in maps_list:
            m_score = m.get("score") or {}
            t1 = m_score.get("team1_score") if m_score.get("team1_score") is not None else m_score.get("team1", 0)
            t2 = m_score.get("team2_score") if m_score.get("team2_score") is not None else m_score.get("team2", 0)
            if t1 > t2: score_a += 1
            elif t2 > t1: score_b += 1
        ...
        for map_data in maps_list:
            ...
            m_score = map_data.get("score") or {}
            m_score_t1 = m_score.get("team1_score") if m_score.get("team1_score") is not None else m_score.get("team1", 0)
            m_score_t2 = m_score.get("team2_score") if m_score.get("team2_score") is not None else m_score.get("team2", 0)
            ...
            for p in p_list:
                ...
                if not p_name:
                    continue
                p_team = p.get("team", "")
                weight = match_team(p_team, team_a_name, team_b_name)
                opp_team = team_b_name if weight == 1 else team_a_name
                delta_pts = delta_pts_team1 if weight == 1 else delta_pts_team2
                
                kills = int(p.get("kills") or 0)
                if kills == 0:
                    est_k = self.baseline_lookup.get(p_name, {}).get("kpr", 0.72) * 24
                    kills = int(est_k)
                players_data[p_name]["kills"].append(kills)
                
                rating = float(p.get("rating") or 0.0)
                if rating == 0.0:
                    rating = self.baseline_lookup.get(p_name, {}).get("rating", 1.05)
                players_data[p_name]["ratings"].append(rating)
                
                kills_pts = self.calculate_kills_points(kills)
                multikill_pts = self.calculate_multikill_points(0, 0, 0, 0)
                
                map_score = kills_pts + multikill_pts + delta_pts
                players_data[p_name]["team"] = p_team or (team_a_name if weight == 1 else team_b_name)
...
def compute_all_players_historical_stats(raw_dir: str = RAW_DIR) -> dict[str, dict]:
    ...
    global _historical_stats_cache
    if _historical_stats_cache is not None:
        return _historical_stats_cache
...
def optimize_roster(
    ...
    for p in vfl_players:
        ...
        if wr >= survival_threshold or is_in_curr:
            ...
            # If team is inactive in this Gameweek and player is NOT in current_roster (transfer constraint),
            # exclude from incoming transfer candidates entirely!
            if not is_active and transfer_constraint and not is_in_curr:
                continue

            if not is_active:
                p_norm["computed_ppg"] = 0.0
                p_norm["global_ppg"] = 0.0
                ...
                if has_precomputed:
                    ...
                    if p_norm.get("computed_ppg") is not None:
                        global_ppg = float(p_norm["computed_ppg"])
                    elif p_norm.get("EV") is not None:
                        global_ppg = float(p_norm["EV"])
                    elif p_norm.get("ppg") is not None:
                        global_ppg = float(p_norm["ppg"])
                    elif stats.get("ppg") is not None:
                        global_ppg = float(stats["ppg"])
                    else:
                        global_ppg = 10.0
    ...
    for opt_idx in range(3):
        ...
        if result is not None and result["solver_status"] == "optimal":
            ...
            recommendations.append({
                ...
                "net_ev_gain": round(max(projected_gain, 0.0), 2),
                "new_roster_ev": round(result["projected_points"], 2)
            })
        else:
            break
