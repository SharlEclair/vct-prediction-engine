...
def generate_v9_optimal_roster(
    ...
    1. Step A (v9_historical_stats.py): Replaces legacy unweighted PPG with Bayesian decay stats,
       Agent Mastery Inertia buffer for patch shocks, role-normalized telemetry modifiers
       (ADR, KAST%, FD), and Skill-Ceiling Elasticity for CVaR_90.
    ...
    for p in raw_players:
        ...
        # Phase 1: Compute Agent Mastery Inertia if patch shock / agent is present
        p_agent = str(p.get("agent") or p.get("agent_name") or p.get("signature_agent") or "").strip()
        p_shock = float(p.get("patch_shock", p.get("concept_drift", 0.0)))
        mastery_idx = calculate_mastery_index(
            player_name=pname,
            agent_name=p_agent,
            telemetry_db=p.get("telemetry_db") or p,
            maps_played=p.get("agent_maps_played")
        ) if p_agent else 0.0
        ...
        # Telemetry-Modified EV & CVaR bounds (with Phase 3 Skill-Ceiling Elasticity)
