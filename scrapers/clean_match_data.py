...
def clean_advanced_stats(adv_list: list, known_players: list[str] = None) -> bool:
    modified = False
    for row in adv_list:
        if not isinstance(row, dict):
            continue
        keys_to_remap = [k for k in row.keys() if k in ADV_STAT_KEY_MAP]
        if keys_to_remap:
            modified = True
            for old_k in keys_to_remap:
                new_k = ADV_STAT_KEY_MAP[old_k]
                row[new_k] = row.pop(old_k)

        p_name = row.get("player", "")
        if p_name and known_players:
            matched = False
            for kp in known_players:
                if kp and p_name.lower().startswith(kp.lower()) and p_name != kp:
                    row["player"] = kp
                    modified = True
                    matched = True
                    break
            if not matched and len(p_name.split()) > 1:
                parts = p_name.split()
                if len(parts[-1]) <= 4 and parts[-1].isupper():
                    row["player"] = " ".join(parts[:-1])
                    modified = True
        elif p_name and len(p_name.split()) > 1:
            parts = p_name.split()
            if len(parts[-1]) <= 4 and parts[-1].isupper():
                row["player"] = " ".join(parts[:-1])
                modified = True
...
def clean_single_file(fpath: str):
    ...
    if data.get("schema_version") == "1.0":
        # Schema v1.0 outputs are natively sanitized and immutable in data/raw/
        return False
    ...
    if is_new_schema:
        ...
        for raw_map in raw_segments:
            ...
            # 4. Clean players stats & advanced stats
            players = raw_map.get("players", [])
            for p in players:
                clean_player_dict(p)
                modified = True

            adv_list = raw_map.get("performance", {}).get("advanced_stats", [])
            known_players = []
            if isinstance(players, list):
                for p in players:
                    if isinstance(p, dict) and p.get("name"):
                        known_players.append(p.get("name"))
                    elif isinstance(p, dict) and p.get("player"):
                        known_players.append(p.get("player"))

            if adv_list and clean_advanced_stats(adv_list, known_players):
                modified = True
                
            # 5. Clean patch and date
            date_str = raw_map.get("date", "")
            patch_val = raw_map.get("patch")

            if date_str and "Patch" in date_str:
                patch_match = re.search(r'Patch\s+([0-9.]+)', date_str, re.IGNORECASE)
                if patch_match:
                    extracted_patch = patch_match.group(1).strip()
                    if not patch_val or patch_val == "Unknown":
                        raw_map["patch"] = extracted_patch
                        patch_val = extracted_patch
                        modified = True
                    clean_date = re.sub(r'\s*Patch\s+[0-9.]+', '', date_str, flags=re.IGNORECASE).strip()
                    if clean_date != date_str:
                        raw_map["date"] = clean_date
                        modified = True

            if not raw_map.get("patch") or raw_map.get("patch") == "Unknown":
                vetoes = raw_map.get("vetoes", [])
                found_p = None
                for v in vetoes:
                    pm = re.search(r'Patch\s+([0-9.]+)', str(v), re.IGNORECASE)
                    if pm:
                        found_p = pm.group(1).strip()
                        break
                if found_p:
                    raw_map["patch"] = found_p
                    modified = True
                elif raw_map.get("patch") == "Unknown":
                    raw_map["patch"] = None
                    modified = True
        ...
        if len(teams) == 2:
            ...
            patch_val = segment.get("patch")
            if date_str and "Patch" in date_str:
                patch_match = re.search(r'Patch\s+([0-9.]+)', date_str, re.IGNORECASE)
                if patch_match:
                    extracted_patch = patch_match.group(1).strip()
                    if not patch_val or patch_val == "Unknown":
                        segment["patch"] = extracted_patch
                        patch_val = extracted_patch
                        modified = True
                    clean_date = re.sub(r'\s*Patch\s+[0-9.]+', '', date_str, flags=re.IGNORECASE).strip()
                    if clean_date != date_str:
                        segment["date"] = clean_date
                        modified = True
