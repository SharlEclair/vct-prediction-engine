    ...
    for f in files:
        ...
        try:
            ...
            segment = {
                ...
                "teams": [
                    {"name": norm["teams"]["team1"], "score": norm["score"].get("team1_score", 0)},
                    {"name": norm["teams"]["team2"], "score": norm["score"].get("team2_score", 0)}
                ],
                "maps": norm["maps"],
...
def get_latest_roster(team_name: str, raw_dir: str, exclude_match_ids: list = None) -> list[str]:
    ...
    for f in files:
        ...
        try:
            with open(f, "r", encoding="utf-8") as file:
                content = json.load(file)
                norm = normalize_match(content)
                if str(norm.get("match_id")) in exclude_set:
                    continue
                ta = norm.get("team_a", "")
                tb = norm.get("team_b", "")
                ts = norm.get("timestamp") or parse_match_date(norm.get("date", ""))
                weight = match_team(team_name, ta, tb)
                if weight != 0:
                    matches_with_team.append((ts, norm, ta, tb, weight))
        except Exception as e:
            pass
    ...
    roster = set()
    maps_list = latest_segment.get('maps', [])
    if isinstance(maps_list, dict):
        maps_list = list(maps_list.values())
    for map_data in maps_list:
        if isinstance(map_data, dict):
            players = map_data.get('players', {})
            if isinstance(players, dict):
                p_list = players.get(team_key, [])
            ...
            for p in p_list:
                if isinstance(p, dict) and 'name' in p:
                    p_name = p['name']
                    if "inactive" not in p_name.lower():
                        roster.add(p_name)
...
def predict_grand_final(target_match_id: str = None):
    match_id_to_predict = target_match_id or TARGET_MATCH_ID
    logger.info(f"Starting real-time prediction orchestrator for match {match_id_to_predict}...")
    
    # 1. Load match details
    match_file = os.path.join(RAW_DIR, f"match_{match_id_to_predict}.json")
    if not os.path.exists(match_file):
        raise FileNotFoundError(f"Match file not found at {match_file}")
        
    with open(match_file, "r", encoding="utf-8") as f:
        match_data = json.load(f)
        
    norm = normalize_match(match_data)
    team_a_name = norm.get("team_a", "Team 1")
    team_b_name = norm.get("team_b", "Team 2")
    
    logger.info(f"Target Match: {team_a_name} vs {team_b_name} (ID: {match_id_to_predict})")
    
    # 2. Roster Identification across all map segments
    roster_a = set()
    roster_b = set()
    for map_data in norm.get("maps", []):
        raw_players = map_data.get('players', [])
        if isinstance(raw_players, list):
            for p in raw_players:
                p_name = p.get('name')
                p_team = p.get('team', '')
                if p_name and match_team(p_team, team_a_name, team_b_name) == 1:
                    roster_a.add(p_name)
                elif p_name and match_team(p_team, team_a_name, team_b_name) == -1:
                    roster_b.add(p_name)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Predict VCT Match")
    parser.add_argument("--match_id", type=str, default="706371", help="Match ID to predict")
    args = parser.parse_args()
    predict_grand_final(target_match_id=args.match_id)
